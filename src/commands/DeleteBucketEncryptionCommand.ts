...
import { DeleteBucketEncryptionCommand as __AWS__DeleteBucketEncryptionCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__DeleteBucketEncryptionCommand(this.input));
    }
