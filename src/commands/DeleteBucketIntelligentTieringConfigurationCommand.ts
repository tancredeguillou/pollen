...
import { DeleteBucketIntelligentTieringConfigurationCommand as __AWS__DeleteBucketIntelligentTieringConfigurationCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__DeleteBucketIntelligentTieringConfigurationCommand(this.input));
    }
