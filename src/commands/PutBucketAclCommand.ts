...
import { PutBucketAclCommand as __AWS__PutBucketAclCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketAclCommand(this.input));
    }
