...
import { PutBucketCorsCommand as __AWS__PutBucketCorsCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketCorsCommand(this.input));
    }
