...
import { PutBucketEncryptionCommand as __AWS__PutBucketEncryptionCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketEncryptionCommand(this.input));
    }
