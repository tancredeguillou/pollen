...
import { PutBucketIntelligentTieringConfigurationCommand as __AWS__PutBucketIntelligentTieringConfigurationCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketIntelligentTieringConfigurationCommand(this.input));
    }
