...
import { PutBucketLifecycleConfigurationCommand as __AWS__PutBucketLifecycleConfigurationCommand } from "@aws-sdk/client-s3";

import { PutBucketLifecycleConfigurationRequest, PutBucketLifecycleConfigurationOutput } from "../models/models_0";
 ...
 *                   related subresources (for example, lifecycle configuration and LifecycleConfiguration
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketLifecycleConfigurationCommand(this.input));
    }
