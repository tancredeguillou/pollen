...
import { PutBucketLoggingCommand as __AWS__PutBucketLoggingCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketLoggingCommand(this.input));
    }
