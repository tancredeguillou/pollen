...
import { PutBucketReplicationCommand as __AWS__PutBucketReplicationCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketReplicationCommand(this.input));
    }
