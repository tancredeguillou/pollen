...
import { PutBucketRequestPaymentCommand as __AWS__PutBucketRequestPaymentCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketRequestPaymentCommand(this.input));
    }
