...
import { PutBucketTaggingCommand as __AWS__PutBucketTaggingCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketTaggingCommand(this.input));
    }
