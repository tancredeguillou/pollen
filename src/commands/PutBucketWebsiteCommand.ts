...
import { PutBucketWebsiteCommand as __AWS__PutBucketWebsiteCommand } from "@aws-sdk/client-s3";
...
> {
    ...
    async resolve(providers: Providers): Promise<void> {
        await providers.getAwsProvider().client.send(new __AWS__PutBucketWebsiteCommand(this.input));
    }
